<template>
    <div class="row" id="categorySummaryCards">
        <div v-for="category in categories" :key="category.id" >
            <CategoryBalanceCard v-bind:categoryName="category.name" v-bind:categoryBalance="category.balance"/>
        </div>        
    </div>
</template>
    ...
    mounted() {
        ...
        categoriesService.getCategories().then((categories) => {
            categories.forEach((category) => {
                transactionsService.getFinancialReport(null, null, null, category.id).then((financialReport) => {
