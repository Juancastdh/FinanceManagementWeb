...
function getCategoryById(id: number): Category {
  let category = new Category(0,"",0);
  let foundCategory = props.categories.find((cat) => cat.id == id);
  ...
  transactionsTable.value = $('#transactionsTable').DataTable({
    ...
    columns: [
      ...
      },
      {
        data: 'categoryId',
        render: function (data: number) {
          let category = getCategoryById(data)
...
async function getFinancialReportBasedOnCurrentFilter(): Promise<FinancialReport> {
  ...
  if (currentFilter.value.id == 'filter-period') {
    financialReport = await transactionsService.getFinancialReport(
      latestPeriod.value.id,
      null,
      null,
      null
    )
